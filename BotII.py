...
def calculate_strategy(candles):
    ...
    # Entry conditions
    df['bb_breakout_up'] = (df['close'] > df['bb_upper'].shift(1)) & (df['close'].shift(1) <= df['bb_upper'].shift(1))
    df['bb_breakout_down'] = (df['close'] < df['bb_lower'].shift(1)) & (df['close'].shift(1) >= df['bb_lower'].shift(1))
    df['macd_crossover_up'] = (df['macd'] > df['macd'].shift(1)) & (df['macd'].shift(1) <= df['macd_signal'].shift(1))
    df['macd_crossover_down'] = (df['macd'] < df['macd'].shift(1)) & (df['macd'].shift(1) >= df['macd_signal'].shift(1))
    df['is_oversold'] = df['%K'] < 20
    df['is_overbought'] = df['%K'] > 80

    # Exit conditions
    df['exit_conditions'] = (df['macd'] > df['macd_signal']) & (df['macd'].shift(1) <= df['macd_signal'].shift(1))

    # Apply the trading strategy
    df['signal'] = ''
    df.loc[df['bb_breakout_up'] & df['macd_crossover_up'] & df['is_oversold'], 'signal'] = 'Buy'
    df.loc[df['bb_breakout_down'] & df['macd_crossover_down'] & df['is_overbought'], 'signal'] = 'Sell'
...
def backtest(strategy_data):
    ...
    for i in range(1, len(strategy_data)):
        ...
        if current_signal == 'Buy' and previous_signal != 'Buy':
            ...
            trades.append((strategy_data.loc[i, 'timestamp'], 'Buy', position, close_price))

        elif current_signal == 'Sell' and previous_signal != 'Sell':
            # Exit long position
            if position > 0:
                capital = position * close_price
                trades.append((strategy_data.loc[i, 'timestamp'], 'Sell', position, close_price))
                position = 0

    if position > 0:
        # Sell at the last close price if still in position
        capital = position * close_price
        trades.append((strategy_data.loc[len(strategy_data) - 1, 'timestamp'], 'Sell', position, close_price))
        position = 0
